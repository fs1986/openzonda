...
Producto: OpenZonda · Key Jira: OPENZONDA (tarjetas OZ-N) · Repo: openzonda (monorepo: código y documentación).
...
| Trazabilidad | Cada sesión genera un session log en `docs/sessions/`, enlazado a su tarjeta |
...
| scrum | Retro de fase, métricas de sesiones (duración, DoD fallidos), detección de scope creep | Read (`docs/sessions/`, `docs/retros/`) | Cierre de cada fase |
...
- Contexto: enlaces a Diseño v0.2, Plan v1.0 y este documento en `docs/design/`; resumen de arquitectura de capas y decisiones inmutables (§25 del diseño) que ningún agente puede violar.
- Flujo obligatorio de sesión (protocolo §3 de este documento) incluido literalmente para que Code lo siga sin recordárselo.
- Convenciones: commits convencionales (feat/fix/docs/test/chore + scope de paquete), rama por tarjeta (feature/oz-N-slug), PR aunque seas el único humano (la revisión la hace architect+qa+security antes de pedirte merge).
...
# 2. Documentación: dónde vive
La documentación es la memoria del proyecto y el registro auditable de cada sesión. Vive **dentro del monorepo**, junto al código.

> **Corrección respecto a v1.0 de este plan (registrada en OZ-31, 2026-08-02).** La versión original recomendaba un repositorio Git separado, `openzonda-docs`, para que su historia no se mezclara con la del código y pudiera compartirse sin dar acceso al código en fases tempranas. **Ese repositorio nunca se creó** y la documentación se escribió desde el principio dentro del monorepo.
>
> El desvío no se registró en su momento y el documento siguió describiendo una estructura inexistente durante toda la fase F0, con un coste medible: el DoD de OZ-1 exigía carpetas `retros/` y `templates/` que nadie creó, en parte porque la ruta citada apuntaba a un repositorio que no existía. Varias tarjetas del catálogo §5 heredaron rutas `openzonda-docs/...` igualmente inválidas.
>
> **Se adopta la estructura real.** El argumento del repo separado —poder compartir la documentación sin dar acceso al código— dejó de aplicar cuando el repositorio de código se hizo público bajo Apache-2.0: ya no hay nada que proteger separando historias. A cambio, el monorepo hace que un cambio de diseño y su implementación viajen en el mismo commit y en el mismo PR, que es justo lo que hace auditable la trazabilidad tarjeta → decisión → código.

openzonda/
  docs/
    design/
      software-design-v0.2.md  # diseño (fuente de verdad)
      plan-implementacion.md
      plan-operativo-sesiones.md   # este documento
    sessions/                  # UN archivo por sesión
      S001_OZ-1_bootstrap-documental.md
      S002_OZ-2_monorepo.md
      ...
    retros/                    # retro de cada fase (agente scrum)
      F0-retro.md
    templates/
      session-log.md  jira-card.md  retro.md
  ADR/                         # ADR-001..N (inmutables), en la raíz
  README.md                    # visión, instalación y estado del proyecto
  CHANGELOG.md                 # qué cambió, orientado a quien usa OpenZonda

Dos detalles de la estructura real que difieren de lo que preveía v1.0:

- **`ADR/` está en la raíz**, no bajo `docs/`. Se mantiene así porque `CLAUDE.md` y varios documentos ya referencian esa ruta, y moverla rompería enlaces por un beneficio puramente cosmético.
- **`hardware/HARDWARE.md` todavía no existe.** Se creará en F2, que es cuando habrá NICs validadas que registrar. No tiene sentido antes.
...
| 6. Documentación | docs | Actualiza docs afectadas, CHANGELOG y genera el session log en `docs/sessions/` |
...
Objetivo: Crear la estructura documental de §2 dentro del monorepo, migrar diseño y planes a markdown, ADR-001..007, templates.
Subagentes: docs (líder), pm-jira, architect (valida ADRs migrados)
Instrucción de arranque para Claude Code:
Sesión S001 / OZ-1. Crea el repo documental según §2 del plan
...
Instrucción de arranque para Claude Code:
Sesión S002 / OZ-2. Crea el monorepo según §7.3 del diseño
...
Instrucción de arranque para Claude Code:
Sesión S003 / OZ-3. Implementa MainWindow mínima con
...
Instrucción de arranque para Claude Code:
Sesión S004 / OZ-4. Autoría WiX v5: MSI per-user,
...
Instrucción de arranque para Claude Code:
Sesión S005 / OZ-5. Implementa el dominio de F1.1 del
...
Instrucción de arranque para Claude Code:
Sesión S006 / OZ-6. Runner de migraciones minimalista y
...
### S007 · [OZ-7] Contenedor .wifisurvey
...
Instrucción de arranque para Claude Code:
Sesión S007 / OZ-7. Implementa el contenedor de proyecto:
...
Instrucción de arranque para Claude Code:
Sesión S008 / OZ-8. Shell según F1.4: docks, patrón
...
### S009 · [OZ-9] Visor de plano + calibración
...
Instrucción de arranque para Claude Code:
Sesión S009 / OZ-9. Visor QGraphicsView (zoom rueda,
...
### S010 · [OZ-10] i18n + release 0.1.0-alpha.1 + retro F1
...
Instrucción de arranque para Claude Code:
Sesión S010 / OZ-10. Externaliza strings a tr(), genera
...
DoD de la sesión: [HW] Instalas alpha.1 desde el MSI de release en VM; retro F1 en docs/retros/.

## 5.3 Fase F2 — Captura Native WiFi (7 sesiones)

### S011 · [OZ-11] Bindings ctypes de wlanapi
...
Instrucción de arranque para Claude Code:
Sesión S011 / OZ-11. Implementa packages/wifi/win32/:
...
### S012 · [OZ-12] Flujo de scan + notificaciones
...
Instrucción de arranque para Claude Code:
Sesión S012 / OZ-12. Implementa el flujo completo de
...
### S013 · [OZ-13] CLI oz-capture + primeros fixtures
...
Instrucción de arranque para Claude Code:
Sesión S013 / OZ-13. CLI oz-capture: ejecuta el flujo,
...
### S014 · [OZ-14] Parser de IEs (núcleo)
...
Instrucción de arranque para Claude Code:
Sesión S014 / OZ-14. Parser TLV robusto según §4.4 del
...
### S015 · [OZ-15] Parser de IEs (VHT/HE/EHT) + derivación canal/ancho
...
Instrucción de arranque para Claude Code:
Sesión S015 / OZ-15. Completa el parser para WiFi
...
### S016 · [OZ-16] health() + netsh fallback
...
Instrucción de arranque para Claude Code:
Sesión S016 / OZ-16. Implementa ScannerHealth con las 5
...
### S017 · [OZ-17] Panel de scan en vivo + retro F2
...
Instrucción de arranque para Claude Code:
Sesión S017 / OZ-17. Scanner thread con cola thread-safe
...
| --- | --- | --- | --- |
| S018 | OZ-18 | SurveySession + Measurement + flags de calidad + adapter_profile | Sesión persiste y reabre; muestras inmutables |
| S019 | OZ-19 | Flujo stop-and-go: clic → N scans → punto; atajos espacio/Z | [HW] Survey de 20 puntos en tu casa/oficina |
| S020 | OZ-20 | Capa de muestras sobre plano coloreada por RSSI del SSID activo | Feedback visual inmediato por punto |
| S021 | OZ-21 | Export CSV/JSON + perfiles de adaptador en reporte | Schema JSON publicado en docs |
| S022 | OZ-22 | Kit de alpha: guía 1 página, Discussions, release 0.1.0-alpha.3, retro F3 | [HW] Envías el kit a 3–5 colegas — hito ALPHA |
...
- Registrar la marca digital (verificado 02-08-2026: sin paquete PyPI 'openzonda'; openzonda.org/.com/.dev/.io/.cl sin DNS — confirmar en el registrador antes de pagar). **Estado real al 02-08-2026**: la organización de GitHub `openzonda` **está creada** y el repositorio vive en `github.com/openzonda/openzonda`, transferido desde la cuenta personal del fundador; el proyecto Jira existe con tablero Kanban y tarjetas `OZ-N`. Sigue **pendiente** registrar los dominios y reservar el nombre en PyPI.
- Instalar en el equipo de desarrollo: Git, uv, .NET SDK 8 y WiX v5 (solo para el MSI), `gh` y Claude Code; verificar acceso MCP Atlassian desde Code. **No hace falta instalar Python**: uv descarga la versión correcta. Tampoco Node: v1.0 del plan lo listaba «para tooling», pero el proyecto no usa nada de Node.
- Pedir a Claude (chat) la creación del primer conjunto de tarjetas (OZ-1..22) desde el catálogo §5 — cuando tú lo decidas.
...
Fin del plan operativo — v1.1. Vive en `docs/design/` dentro del monorepo y se actualiza por ADR.
